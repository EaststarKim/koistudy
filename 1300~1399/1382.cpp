...
int g[110],chk[2010],st[110],n,m,cnt,lca;
int q[110],l[110],p[110],t[110],w[110],spt[110][7];
long long ans[2010],sum;
...
int LCA(int s,int e){
    int i;
    if(l[s]<l[e])swap(s,e);
    for(i=6;i>=0;--i)if(l[s]-l[e]>=1<<i)s=spt[s][i];
    if(s==e)return s;
    for(i=6;i>=0;--i)if(spt[s][i]!=spt[e][i])s=spt[s][i],e=spt[e][i];
    return p[s];
}
void f(int s,int W){
    s=seek(s);
